...
import com.rss.espn.rpm.ObjectFactory;
import com.rss.espn.rpm.Rss;

@Component
public class EspnRpmParser {

	String fromFile = "http://www.espn.com/espn/rss/rpm/news";
	String toFile = "xmls/espnrpm.xml";

	public List<RssItemDto> EspnRpmParse() {
		try {
	...
	public List<RssItemDto> espnRpmUnmarshaller() {
		try {

			File file = new File("xmls/espnrpm.xml");
	...
	public List<RssItemDto> parsher(Rss rss) {
		...
		if (rss.getChannel() != null) {
			for (Rss.Channel.Item rci : rss.getChannel().getItem()) {
				...
				ri.setCatagory(SportCat.MotorSports);
				ri.setSource("ESPN");
